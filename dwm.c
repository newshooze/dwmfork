...
#define TOGGLE_MUTE             1 << 8
#define MAX_DATE_LENGTH         0xff
/* enums */
...
static void changeaudiosetting(const Arg *arg);
...
static void showcalendar(const Arg *arg);
...
buttonpress(XEvent *e)
{
	...
	{
		int clockwidth = TEXTW(clockstring);
		for(i=0,x=0; i<LENGTH(tags); i++)
		{
			int step = TEXTW(tags[i]);
			if(ev->x > x && ev->x < x + step)
			{
				click = ClkTagBar;
				arg.ui = 1 << i;
			}
		...
		else if(ev->x > m->ww - clockwidth - volumetraywidth - bh && ev->x < m->ww - clockwidth - volumetraywidth)
			click = ClkTray0;
		/* volume adjustment / mute */
		else if(ev->x > m->ww - clockwidth - volumetraywidth && ev->x < m->ww - clockwidth)
			click = ClkVolume;
		else if(ev->x > m->ww - clockwidth)
			click = ClkClock;
	} 
...
/* vz */
/* arg->i can be positive or negative */
/* arg->ui toggles mute with a constant TOGGLE_MUTE*/
void
changeaudiosetting(const Arg *arg)  
{
	if(!arg)
		return;
...
drawbar(Monitor *m)
{
	...
	/* Draw volume tray */
	drw_rect(drw,x,4,volumetraywidth,bh-8,1,0);
	if(pulsemuted())
	{
		drw_text(drw,x,4,volumetraywidth,bh-8,2,"MUTED",1);
	}
	else
	{
		/* Draw volume slider */
...
/* this is broken for certain colors */
void
printscreen(const Arg *arg)
{
#define MAX_FNAME 0xff
	int ret;
	...
	XpmWriteFileFromImage(display,getcurrenttimestring(fname,MAX_FNAME),img,NULL,NULL);
...
run(void)
{
	...
	/* main event loop */
	int eventspending;
	int x11connection = ConnectionNumber(dpy);
	XSync(dpy, False);
	while (running)
	{
...
#define CLOCK_UPDATE_INTERVAL 30 
		...
		/* set timer */
		eventspending = select(x11connection + 1,&infiledescriptor,NULL,NULL,&timer);
		if(eventspending == 0) // No events pending
		{
			gettimestringpretty(clockstring,MAX_DATE_LENGTH);
			drawbar(selmon);
		}
		while(XPending(dpy)) // int
		{
			XNextEvent(dpy,&ev); // int
			if(handler[ev.type])
			{
				handler[ev.type](&ev);
			}
...
togglebarpos(const Arg *arg)
{
	...
	updatebarpos(selmon);
	XMoveResizeWindow(dpy,selmon->barwin,selmon->wx,selmon->by,selmon->ww,bh); // int
	arrange(selmon);
}
